...
fn git_status(repo_path: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn git_push(repo_path: &str, remote: &str, branch: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let diff_output =  Command::new("git").current_dir(repo_path).args(&["add", "./*"]).output();
    match diff_output {
        Ok(op) => {
            println!("Git OP::>{:?}", op);
            let commit = command.args(&["commit", "-m", "committed by code"])
                .output();
            match commit {
                Ok(commit_op) => {
                    println!("Git commit :{:?}", commit_op);
                    let push_output =  Command::new("git").current_dir(repo_path).args(&["push", "-v", remote, branch]).output()?;
